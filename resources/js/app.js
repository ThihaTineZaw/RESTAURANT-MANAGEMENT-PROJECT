...
`;
    ...
    $(document).on('click', '.qty-increase', function (e) {
        ...
        const OriginalPrice = $(this).closest('tr').find('.price-value').attr('original-price');
        const newPrice = OriginalPrice * newQty;
        $(this).closest('tr').find('.price-value').text(newPrice + ' Ks');
    ...
    $(document).on('click', '.menu-item, .qty-increase, .qty-decrease, .delete-order-menu', function () {
        ...
        orderDetails: [
            ...
            axios.post('/cashier/order', orderDetail,
                ...
                if (response.data == 'Order placed successfully') {
                    ...
                    $('#order_id').attr('value', '');
                    updateTotal();
                    $.get('/cashier/orderCheck/' + tableId, function (data) {
                        $('#order-list').html(data.html);
                        $('#order_id').attr('value', data.order_id);
                         $('.table-btn')
                    .removeClass('bg-green-50 dark:bg-green-900/20 border-green-500 text-green-700 dark:text-green-400 hover:bg-green-100 dark:hover:bg-green-900/30');
    ...
    $('#order-again-btn').off('click').on('click', function (e) {
        ...
        $('#order-again-confirm').removeClass('hidden');
            ...
            $('#order-again-cancel-btn').off('click').on('click', function (e) {
                e.preventDefault();
            ...
            $('#order-again-confirm-btn').off('click').on('click', function (e) {
                ...
                axios.post('/cashier/orderAgain/' + orderId, orderAgain,
                    {
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        }
                    ...
                    if (response.data.message == 'Order again placed successfully') {
                        alert('Order again placed successfully');
                        $('#order-again-confirm').addClass('hidden');
                         $.get('/cashier/orderCheck/' + response.data.table_number, function (data) {
                            $('#order-list').html('');
                            $('#order-list').html(data.html);
                            $('#order_id').attr('value', data.order_id);
                            updateTotal();
